...
if __name__ == "__main__":
    ...
    from stonefish.model import T5Model
    from stonefish.language import CommonGen, CommonGenEval
    ...
    model.load_state_dict(torch.load("/tmp/model_3.pth"))
    ...
    def eval_collate_fn(batch):
        ...
        concepts = [" ".join(c) for c in concepts]
        concepts = tokenizer(concepts, padding=True, return_tensors="pt")
        return concepts, list(targets)
    ...
    def fake_eval(model, test_dl, train_fn):
        ...
        for (batch_idx, (encoding, targets)) in enumerate(test_dl):
            ...
            decoded = [[] for _ in range(input_ids.shape[0])]
            for _ in range(3):
                outputs = model.generate(
                    input_ids=input_ids, attention_mask=attention_mask, max_length=32
                )
                ...
                for i in range(input_ids.shape[0]):
                    decoded[i].append(str_outputs[i])
        ...
        scores = scorer(predictions=generated, references=references)
        print(scores)
        return 0.0, 0.0

    ctx = TrainingContext(fake_eval, compute_loss, train_dl, test_dl)
    # ctx = TrainingContext(fake_eval, pg_loss, train_dl, test_dl)
